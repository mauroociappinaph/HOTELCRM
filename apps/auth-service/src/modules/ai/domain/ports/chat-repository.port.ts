...
export abstract class ChatRepositoryPort implements IChatRepository {
  ...
  abstract create(entity: Omit<ChatSession, 'id' | 'createdAt' | 'updatedAt' | 'lastActivity'>): Promise<ChatSession>;
  ...
  abstract saveMessage(sessionId: string, message: Omit<ChatMessage, 'id' | 'createdAt' | 'updatedAt'>): Promise<void>;
  abstract getConversationHistory(sessionId: string, options?: ConversationOptions): Promise<ChatMessage[]>;
  ...
  abstract saveRecommendation(recommendation: Omit<AiRecommendation, 'id' | 'createdAt' | 'updatedAt'>): Promise<void>;
}
